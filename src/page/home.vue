...
const directionArray = [Direction.X, Direction.Y, Direction.XY, Direction.YX, Direction.OVER];

// 判断棋子与现存棋子数组中是否相连
function chessLink(x: number, y: number, state: 0 | 1) {
    const { chessboard } = checkerboardParameter;

    return chessboard.some(item => item.includes(`{"x": ${x}, "y": ${y}, "state": ${state}}`))
}

// 胜利守则
function victoryCode({ x, y, state, array = [], base = 0 }:victoryType): boolean {
    if (array.length === 5) {
